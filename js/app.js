...
function isCollide(snake){
    // if snake bit himself
    for (let i = 1; i < snakeArr.length; i++) {
        if(snake[i].x === snake[0].x && snake[i].y === snake[0].y){
            return true;
        }
    }
    // If you collid to wall
...
function gameEngine(){
    // Part 1 to updating the snake array
    ...
    // if you have eaten the food, increment the score and regenerate the food on screen
    ...
    // Part 2 to display the snake on screen as an HTML element
    board.innerHTML = "";
    ...
    // Display the food on screen as an HTML element
...
window.addEventListener('keydown', e=>{
    inputDirection = {x: 0,y: 1}   // Game starts when a key is pressed
    moveSound.play();
    switch (e.key) {
        case "ArrowUp":
            console.log("ArrowUp");
